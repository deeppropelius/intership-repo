...
const headerTurnTextEl = document.getElementById("headerTurnText");
const headerTurnDotEl = document.getElementById("headerTurnDot");
const centerTurnIndicatorEl = document.getElementById("centerTurnIndicator");
const centerStatusMsgEl = document.getElementById("centerStatusMsg");
const btnRollDiceEl = document.getElementById("btnRollDice");
const btnBuyPropertyEl = document.getElementById("btnBuyProperty");
...
const mobilePlayerStripEl = document.getElementById("mobilePlayerStrip");
...
const modalPreviewColorBarEl = document.getElementById("modalPreviewColorBar");
...
const modalAuthorAvatarEl = document.getElementById("modalAuthorAvatar");
const modalAuthorNameEl = document.getElementById("modalAuthorName");
const modalAuthorRoleEl = document.getElementById("modalAuthorRole");
...
const modalRent1El = document.getElementById("modalRent1");
...
const rowRentBaseEl = document.getElementById("rowRentBase");
const rowRent1El = document.getElementById("rowRent1");
const rowRent2El = document.getElementById("rowRent2");
const rowRent3El = document.getElementById("rowRent3");
const rowRentHotelEl = document.getElementById("rowRentHotel");
...
const setupModalCloseBtnEl = document.getElementById("setupModalCloseBtn");
const btnCancelSetupEl = document.getElementById("btnCancelSetup");
...
function getGridPosition(index) {
    if (index === 0)
        return { row: 10, col: 1, edge: "corner" }; // Start (Bottom-Left)
    if (index >= 1 && index <= 8)
        return { row: 10 - index, col: 1, edge: "edge-left" }; // Left: 1 -> Row 9, ..., 8 -> Row 2
    if (index === 9)
        return { row: 1, col: 1, edge: "corner" }; // Resthouse (Top-Left)
    if (index >= 10 && index <= 17)
        return { row: 1, col: index - 8, edge: "edge-top" }; // Top: 10 -> Col 2, ..., 17 -> Col 9
    if (index === 18)
        return { row: 1, col: 10, edge: "corner" }; // Club (Top-Right)
    if (index >= 19 && index <= 26)
        return { row: index - 17, col: 10, edge: "edge-right" }; // Right: 19 -> Row 2, ..., 26 -> Row 9
    if (index === 27)
        return { row: 10, col: 10, edge: "corner" }; // Jail (Bottom-Right)
...
function updateTokensDisplay() {
    ...
    players.forEach((player) => {
        if (player.isBankrupt)
            return; // Skip eliminated bankrupt players from tile tokens
...
function calculatePlayerPropertyNetWorth(player) {
    return boardSpaces
...
function openPlayerPortfolioModal(player) {
    ...
    if (ownedSpaces.length === 0) {
        ...
        ownedSpaces.forEach((space, idx) => {
            ...
            if (isWhite)
                levelLabel = "Transport";
            else if (space.hasHotel)
                levelLabel = "Hotel (Max)";
            else if (space.houses)
                levelLabel = `${space.houses} House${space.houses > 1 ? 's' : ''}`;
            ...
            cardEl.addEventListener("click", (e) => {
                if (e.target.closest(".acc-inspect-btn")) {
...
function renderPlayers() {
    ...
    if (activePlayer) {
        ...
        if (dockTurnDotEl)
            dockTurnDotEl.style.backgroundColor = activePlayer.color;
    }
...
function openPropertyPreviewModal(space) {
    ...
    if (heroEl)
        heroEl.style.borderColor = groupColor;
    ...
    const imageSrc = space.image || "./assets/Darjeeling.jpg";
    if (heroEl)
        heroEl.style.backgroundImage = `url('${imageSrc}')`;
    const curRent = getCurrentRent(space);
    ...
    if (isWhite) {
        ...
        if (rowRentHotelEl)
            rowRentHotelEl.style.display = "none";
        const rowHouseCostEl = document.getElementById("rowHouseCost");
        if (rowHouseCostEl)
            rowHouseCostEl.style.display = "none";
    }
    else {
        ...
        const rowHouseCostEl = document.getElementById("rowHouseCost");
        if (rowHouseCostEl)
            rowHouseCostEl.style.display = "flex";
        if (modalHouseCostEl) {
    ...
    if (modalSellValueEl)
        modalSellValueEl.textContent = `₹${sellRefund.toLocaleString()}`;
    const owner = players.find(p => p.id === space.ownerId);
...
function calculatePropertyRefund(space) {
...
function resolveDebtAndPay(debtor, creditor, amount, reason) {
...
function calculatePlayerTotalWealth(p) {
    if (p.isBankrupt)
        return 0;
...
function initGame() {
    ...
    document.querySelectorAll(".modal-overlay").forEach(overlay => {
        overlay.addEventListener("click", (e) => {
            if (e.target === overlay) {
                if (overlay.id === "setupModal" && (!isMatchStarted && players.length === 0))
                    return;
